...
export default function Navbar({ onOpenModal }: NavbarProps) {
  ...
  return (
    <nav className="bg-[#FBFAFF] text-[#2B2823] w-full fixed top-0 left-0 z-50 border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden lg:flex items-center gap-10 text-sm font-medium">
            ...
            <Link to="/produto" onClick={closeMenu} className="hover:underline">Produto</Link>
            <Link to="" onClick={closeMenu}>Segmentos</Link>
            <Link to="" onClick={closeMenu}>Planos</Link>
            <Link to="" onClick={closeMenu}>Recursos</Link>
            <Link to="" onClick={closeMenu}>Contato</Link>
          </div>
      ...
      <div
        className={`lg:hidden bg-[#FBFAFF] border-t transition-all duration-300 ${isOpen
